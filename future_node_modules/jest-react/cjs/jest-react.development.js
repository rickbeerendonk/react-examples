...
function assertYieldsWereCleared(root) {
  ...
  !(actualYields.length === 0) ? invariant(false, 'Log of yielded values is not empty. Call expect(ReactTestRenderer).toHaveYielded(...) first.') : void 0;
}

function toFlushAndYield(root, expectedYields) {
...
function toFlushAndYieldThrough(root, expectedYields) {
  return captureAssertion(function () {
    assertYieldsWereCleared(root);
...
function toFlushWithoutYielding(root) {
  return toFlushAndYield(root, []);
}

function toHaveYielded(ReactTestRenderer, expectedYields) {
  return captureAssertion(function () {
    if (ReactTestRenderer === null || typeof ReactTestRenderer !== 'object' || typeof ReactTestRenderer.unstable_setNowImplementation !== 'function') {
      invariant(false, 'The matcher `toHaveYielded` expects an instance of React Test Renderer.\n\nTry: expect(ReactTestRenderer).toHaveYielded(expectedYields)');
    }
...
function toMatchRenderedOutput(root, expectedJSX) {
  ...
  if (jsonChildren !== null) {
    if (jsonChildren.length === 1) {
      ...
      for (var i = 0; i < jsonChildren.length; i++) {
        var jsxChild = jsonChildrenToJSXChildren(jsonChildren[i]);
...
exports.toFlushAndYield = toFlushAndYield;
exports.toFlushAndYieldThrough = toFlushAndYieldThrough;
exports.toFlushWithoutYielding = toFlushWithoutYielding;
exports.toHaveYielded = toHaveYielded;
exports.toFlushAndThrow = toFlushAndThrow;
