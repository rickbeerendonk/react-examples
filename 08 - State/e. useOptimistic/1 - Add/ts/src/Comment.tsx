...
import { delay, saveCommentToServer, type Comment } from './utils/index.ts';

interface CommentsProps {
  initialComments: Comment[];
}

// Client-side optimistic UI state (includes pending status)
...
export default function Comments({ initialComments }: CommentsProps) {
  ...
  >(
    ...
    (state, newComment: string): OptimisticComment[] => [
      ...state,
  ...
  // handleSubmit is an Action - an async function that handles form submission with transitions
  ...
  return (
    <form action={handleSubmit}>
      ...
      <ul>
        ...
        {optimisticComments.map((comment: OptimisticComment, i) => (
